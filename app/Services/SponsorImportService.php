...
class SponsorImportService
{
        ...
        try {
            ...
            DB::transaction(function () use ($tmp, $stats, $advance): void {
                ...
                foreach ($file as $row) {
                    ...
                    $sourceHash = $this->sourceHash($company, $postcode, $licenceNumber);
                    $slug = Sponsor::makeSlug($company, $sourceHash);
    ...
    private function upsert(array $batch): int
    {
        Sponsor::upsert(
            $batch,
            ['source_hash'],
            ['company_name', 'slug', 'town', 'county', 'postcode', 'licence_number', 'organisation_type', 'routes', 'rating', 'status', 'imported_at', 'updated_at']
        );

        return count($batch);
    }

    private function sourceHash(string $company, ?string $postcode, ?string $licenceNumber): string
    {
        return sha1(mb_strtolower(trim($company)) . '|' . mb_strtolower((string) $postcode) . '|' . mb_strtolower((string) $licenceNumber));
    }
