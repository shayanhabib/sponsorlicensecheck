...
{
    $database->exec('CREATE TABLE IF NOT EXISTS sponsors (id INTEGER PRIMARY KEY AUTOINCREMENT, company_name TEXT NOT NULL, slug TEXT NOT NULL UNIQUE, town TEXT, county TEXT, postcode TEXT, licence_number TEXT, organisation_type TEXT, routes TEXT, rating TEXT, status TEXT, imported_at TEXT, created_at TEXT, updated_at TEXT, UNIQUE(company_name, postcode, licence_number))');
...
function import_csv(\SQLite3 $database, string $path): array
{
    ...
    $statement = $database->prepare('INSERT INTO sponsors (company_name, slug, town, county, postcode, licence_number, organisation_type, routes, rating, status, imported_at, created_at, updated_at) VALUES (:company_name, :slug, :town, :county, :postcode, :licence_number, :organisation_type, :routes, :rating, :status, datetime("now"), datetime("now"), datetime("now")) ON CONFLICT(company_name, postcode, licence_number) DO UPDATE SET slug=excluded.slug, town=excluded.town, county=excluded.county, organisation_type=excluded.organisation_type, routes=excluded.routes, rating=excluded.rating, status=excluded.status, imported_at=datetime("now"), updated_at=datetime("now")');
    ...
    foreach ($file as $row) {
        ...
        $values = [
            ...
            ':slug' => slug($company . ' ' . ($postcode ?: $licenceNumber ?: '')),
